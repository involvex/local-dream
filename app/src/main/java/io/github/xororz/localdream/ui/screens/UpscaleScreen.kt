...
private val Unit.select_upscaler: Int
    get() = R.string.select_upscaler
...
fun UpscalerDialog(
    ...
    AlertDialog(
        ...
        title = { Text(stringResource(R.string.select_upscaler)) },
        text = {
            LazyColumn(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                items(upscalers) { upscaler ->
                    Row(
                        modifier = Modifier
                            .fillMaxWidth()
                            .clickable { onSelectUpscaler(upscaler.id) }
                            .padding(8.dp),
                        verticalAlignment = Alignment.CenterVertically
                    ) {
                        RadioButton(
                        ...
                        if (upscaler.id == downloadingUpscalerId && downloadProgress != null) {
                            Spacer(modifier = Modifier.weight(1f))
                            CircularProgressIndicator(
                                progress = { downloadProgress.progress },
                                modifier = Modifier.size(24.dp)
                            )
                        } else if (!upscaler.isDownloaded && upscaler.id != downloadingUpscalerId) {
                            Spacer(modifier = Modifier.weight(1f))
                            IconButton(onClick = { onDownload(upscaler) }) {
